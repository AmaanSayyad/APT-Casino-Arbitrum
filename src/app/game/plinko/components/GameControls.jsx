...
export default function GameControls({ onBet, onRowChange, onRiskLevelChange, onBetAmountChange, initialRows = 16, initialRiskLevel = "Medium" }) {
  ...
  const handleBet = () => {
    ...
    // Safely check window.ethereum without calling isConnected() to avoid MetaMask initialization errors
    let ethereumConnected = false;
    try {
      if (typeof window !== 'undefined' && window.ethereum) {
        const ethereumIsConnected = window.ethereum.isConnected;
        // Only access as property, don't call as function to avoid internal state errors
        if (typeof ethereumIsConnected !== 'function') {
          ethereumConnected = ethereumIsConnected;
        } else {
          // If it's a function, we skip calling it to avoid errors
          // We rely on wagmi's isConnected instead
          ethereumConnected = false; // Will be determined by wagmi's isConnected
        }
      }
    } catch (error) {
    ...
    console.log('🔌 Plinko Bet - Wallet Status:', { 
      ...
      windowEthereumConnected: ethereumConnected,
      windowEthereumAccount: typeof window !== 'undefined' ? window.ethereum?.selectedAddress : undefined
    });
    if (!isConnected) {
